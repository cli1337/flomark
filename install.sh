...
echo ""
read -p "Configure domain? [y/N]: " configure_domain </dev/tty || configure_domain=""
...
if [[ $configure_domain =~ ^[Yy]$ ]]; then
    ...
    echo ""
    read -p "Domain name: " DOMAIN </dev/tty || DOMAIN=""
...
echo ""
read -p "Use custom port? [y/N]: " custom_port_choice </dev/tty || custom_port_choice=""

if [[ $custom_port_choice =~ ^[Yy]$ ]]; then
    echo ""
    read -p "Enter backend port (1024-65535): " BACKEND_PORT </dev/tty || BACKEND_PORT="3000"
...
if check_port $BACKEND_PORT; then
    ...
    echo ""
    read -p "Enter choice [1-3]: " port_choice </dev/tty || port_choice="3"
    ...
    case $port_choice in
        ...
        2)
            ...
            if [ -n "$SUGGESTED_PORT" ]; then
                echo ""
            read -p "Enter port to use (suggested: $SUGGESTED_PORT): " custom_port </dev/tty || custom_port="$SUGGESTED_PORT"
            custom_port=${custom_port:-$SUGGESTED_PORT}
        else
            echo ""
            read -p "Enter port to use: " custom_port </dev/tty || custom_port="3001"
            fi
...
if check_port 80; then
    ...
    read -p "Continue anyway? [y/N]: " continue_choice </dev/tty || continue_choice="n"
...
echo "     ✓ .htaccess support"
    echo ""
read -p "Enter choice [1-2] (default: 1): " choice </dev/tty || choice="1"
choice=${choice:-1}
...
echo ""
read -p "Enable Demo Mode? [y/N]: " demo_choice </dev/tty || demo_choice="n"
DEMO_MODE="false"
...
read -p "Edit .env now? [y/N]: " edit_env </dev/tty || edit_env="n"
...
read -p "First Name: " OWNER_FIRST </dev/tty
read -p "Last Name: " OWNER_LAST </dev/tty
read -p "Email: " OWNER_EMAIL </dev/tty
read -s -p "Password (min 6 chars): " OWNER_PASSWORD </dev/tty
echo ""
